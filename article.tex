...
\noindent When we are looking for neighbours, we need, at first, a list of potential neighbours for each point.
...
\noindent For the purpose of our experiments, i.e. finding \acrshort{bs} neighbours, the previously presented methods are not sufficient to find real neighbours.
...
\noindent Although the filtering methods are providing good results, it is still not enough.
Therefore, we will modify those criteria to take in account the variations of \acrshort{bs} density. Thus, we need some measurement of this density.

As those problems are equivalent and because this is simpler to visualise, we will say that we are looking for detecting the cities.
